...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleCitizenNotFound(
            CitizenNotFoundException ex, HttpServletRequest req) {
        log.info("Citizen not found: {}", ex.getMessage());
        return response(HttpStatus.NOT_FOUND, "Not Found", ex.getMessage(), req);
    }

    @ExceptionHandler(InvalidNationalIdException.class)
    public ResponseEntity<ErrorResponse> handleInvalidNin(
            InvalidNationalIdException ex, HttpServletRequest req) {
        log.info("Invalid NIN: {}", ex.getMessage());
        return response(HttpStatus.BAD_REQUEST, "Invalid National ID", ex.getMessage(), req);
    }

    @ExceptionHandler(RelatedCitizenNotFoundException.class)
    public ResponseEntity<ErrorResponse> handleRelatedCitizenNotFound(
            RelatedCitizenNotFoundException ex, HttpServletRequest req) {
        log.info("Related citizen not found: {}", ex.getMessage());
        return response(HttpStatus.UNPROCESSABLE_ENTITY, "Related Citizen Not Found", ex.getMessage(), req);
    }

    @ExceptionHandler(RegistrationLimitExceededException.class)
    public ResponseEntity<ErrorResponse> handleRegistrationLimit(
            RegistrationLimitExceededException ex, HttpServletRequest req) {
        log.warn("Registration limit: {}", ex.getMessage());
        return response(HttpStatus.CONFLICT, "Registration Limit Exceeded", ex.getMessage(), req);
    }
    ...
    public ResponseEntity<ErrorResponse> handleConstraintViolation(
        ...
        ErrorResponse body = ErrorResponse.builder()
                .status(HttpStatus.BAD_REQUEST.value())
                .error("Validation Failed")
                .message("One or more path or query parameters failed validation")
                .fieldErrors(fieldErrors)
                .path(req.getRequestURI())
                .build();
        return ResponseEntity.badRequest().body(body);
