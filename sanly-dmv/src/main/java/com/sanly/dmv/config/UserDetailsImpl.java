...
public class UserDetailsImpl implements UserDetails {
    ...
    public static UserDetailsImpl from(DmvOfficer officer) {
        List<GrantedAuthority> auths = officer.getRoles().stream()
                .map(SimpleGrantedAuthority::new)
                .map(GrantedAuthority.class::cast)
                .toList();
        return new UserDetailsImpl(
                officer.getOfficerId(), officer.getUsername(), officer.getPassword(),
                officer.getStatus() == OfficerStatus.ACTIVE, auths);
    }
